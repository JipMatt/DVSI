'''
from: https://github.com/SijmeJan/psi_fargo/tree/sjp
...
class Coordinates:
    ...
    def __init__(self, direc):
        ...
        x = np.loadtxt(direc + 'domain_y.dat')
        dx = x[1] - x[0]
        x = x[3:-4]
...
class Fluid:
    ...
    def __init__(self, direc, number=0):
        ...
        if number < 0:
            self.basename = 'tracer' + str(abs(number))
            number = abs(number)
...
class PolyFluid:
    ...
    def __init__(self, direc):
        ...
        for d in data:
            ...
            if d[0].find('INVSTOKEST') != -1:
                tau_t.append(float(d[1]))
                tau.pop()
        tau = np.sort(1.0/np.asarray(tau))
        tau_t = np.sort(1.0/np.asarray(tau_t))

        self.stopping_times = tau
        self.n_dust = len(tau)
        self.n_tracer = len(tau_t)
        ...
        # Tracer fluids
        for m in range(1, self.n_tracer+1):
            self.Fluids.append(Fluid(direc, number=-m))
