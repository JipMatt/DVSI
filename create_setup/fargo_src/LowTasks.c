...
void CreateFields() {
  ...
  Agasx = CreateField("Agasx", 0,0,0,0);
  Agasy = CreateField("Agasy", 0,0,0,0);
#ifdef Z
  Agasz = CreateField("Agasz", 0,0,0,0);
#endif
#endif
...
int RestartSimulation(int n) {
  ...
  __Restart(Vz, n);
#endif
      if(Fluidtype == GAS) __Restart(Energy, n);
#ifdef MHD
  __Restart(Bx, n);
  __Restart(By, n);
  __Restart(Bz, n);
  ...
  if(Fluidtype == GAS)  offset = ParallelIO(Energy, n, MPI_MODE_RDONLY, offset,FALSE);
#ifdef X
