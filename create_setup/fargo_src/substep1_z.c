...
//<#>
  ...
  //CHANGED TO KEEP Vz=0 in equilibrium
	dtOVERrhom = 2.0*dt/(rho[ll]*(zmin(k+1)-zmin(k))+rho[llzm]*(zmin(k)-zmin(k-1)))*(zmed(k)-zmed(k-1));
  //dtOVERrhom = dt/rho[ll];
  
#ifdef CARTESIAN
	vz_temp[ll] = vz[ll];
	if(fluidtype == GAS) vz_temp[ll] -= dtOVERrhom*(p[ll]-p[llzm])/(zmed(k)-zmed(k-1));
	//if(fluidtype == GAS) vz_temp[ll] -= dtOVERrhom*(p[llzp]-p[llzm])/(2*(zmed(k)-zmed(k-1)));
  //if(fluidtype == GAS) vz_temp[ll] -= dtOVERrhom*(p[llzp]-2*p[ll]+p[llzm])/(zmed(k)-zmed(k-1))/(zmed(k)-zmed(k-1));
...
//START OF MY ADDITION TO THE CODE
#ifdef STRATIFIELD 
	if(fluidtype == GAS) {vz_temp[ll] -= OMEGAFRAME*OMEGAFRAME*zmin(k)*dt;}
	if(fluidtype == DUST) {vz_temp[ll] -= OMEGAFRAME*OMEGAFRAME*zmin(k)*dt;}
  if(fluidtype == TRACER) {vz_temp[ll] -= OMEGAFRAME*OMEGAFRAME*zmin(k)*dt;}
#endif //END OF MY ADDITTION TO THE CODE
//END STRATIFIELD

#ifdef ACCELERATIONGAS
	if(fluidtype == GAS) {
  vz_temp[ll] += dt*agasz[ll];
  }
#endif
	...
	vz_temp[ll] = vz[ll];
	if(fluidtype == GAS) vz_temp[ll] -= dtOVERrhom*(p[ll]-p[llzm])/(ymed(j)*(zmed(k)-zmed(k-1)));
