...
#endif
//MY ADDITION
#if defined(STOKESNUMBER) || defined(CONSTANTDUSTSIZE)
      rho_gas = 0.5*(rho[0][l] + rho[0][idm]);
      if ( p > o )  m[p+o*NFLUIDS] = -dt*omega*(sqrt(8/M_PI)*rho_gas/BULKDENSITY)*alpha[p+o*NFLUIDS]*rho_p/rho_o;
      else          m[p+o*NFLUIDS] = -dt*omega*(sqrt(8/M_PI)*rho_gas/BULKDENSITY)*alpha[p+o*NFLUIDS];
#endif
//END OF MY ADDITION
#ifdef CONSTANTDRAG
...
//MY ADDITION
	  ...
	  /* In the line below, the collision term should be
	     ...
	     have the possibility of disabling feedback if necessary.*/
        rho_gas = 0.5*(rho[0][l] + rho[0][idm]);
	  if( q > p ) sum += (sqrt(8/M_PI)*rho_gas/BULKDENSITY)*alpha[q+p*NFLUIDS]*rho_q/rho_p;
	  else        sum += (sqrt(8/M_PI)*rho_gas/BULKDENSITY)*alpha[q+p*NFLUIDS];
#endif
//END OF MY ADDITION
#ifdef CONSTANTDRAG
