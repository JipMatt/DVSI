...
//<#>
	...
	vx_temp[ll] = vx[ll];
	if(fluidtype == GAS) vx_temp[ll] -=  dtOVERrhom*(p[ll]-p[llxm])*Inv_zone_size_xmed(i,j,k);
...
#ifdef SHEARINGBOX
        if (Ny == 1) {
          vx_temp[ll] += (SHEARPARAM-2.0)*dt*OMEGAFRAME*Vy->field_cpu[ll];
        }
#endif

#ifdef ACCELERATIONGAS
	if(fluidtype == GAS) {
  vx_temp[ll] += dt*agasx[ll];
  }
#endif
