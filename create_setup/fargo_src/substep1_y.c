...
//<#>
	...
	vy_temp[ll] = vy[ll];
	if(fluidtype == GAS) vy_temp[ll]-=  dtOVERrhom*(p[ll]-p[llym])/(ymed(j)-ymed(j-1));
	
#ifdef CARTESIAN
#ifdef SHEARINGBOX

	vm = 0.25*(vx_half[ll]+vx_half[llxp]+vx_half[llym]+vx_half[llxp-pitch]);
  // If Ny==1, FARGO forces all ghost zones to the same value
  // This is not good for vx, so calculate manually
  if (Ny == 1) {
    vm = vx_half[ll] + SHEARPARAM*OMEGAFRAME*(ymed(j) - ymin(j));
    // vm = vx_half[ll]+ SHEARPARAM*OMEGAFRAME*ymin(j)
  }
  vy_temp[l] += dt*(2.0*OMEGAFRAME*vm + 2.0*SHEARPARAM*OMEGAFRAME*OMEGAFRAME*ymin(j));

  #ifdef VERTICALSHEAR
    if(fluidtype == GAS) {
    vy_temp[ll] -= 2.0*OMEGAFRAME*OMEGAFRAME*QZ*zmed(k)*dt;
    }
  #endif

  #if defined(DRAGFORCE) || defined(ACCELERATIONGAS)
  //START OF MY ADDITION TO THE CODE
  #ifdef PRESSUREGRADIENT
    if (Fluidtype == GAS) {
      vy_temp[ll] += 0;
    }
  #else
