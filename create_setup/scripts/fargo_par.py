...
def write_par_file(setup_name, pd, shearing_box, output, cfl=0.44, eta=0.05,dust=True, tracer=None, shear=None, nubound=None):
    ...
    # Viscosity using cs=1
    nu = pd.alpha_viscosity_gas
    if pd.alpha_viscosity_dust is None:
        nudust = nu
    else:
        nudust = pd.alpha_viscosity_dust
    ...
    add_line(lines, '\n### Disk parameters\n\n')
    add_line(lines, 'AspectRatio        {:.10f}      Make eta source term 2\n'.format(eta**0.5))
    add_line(lines, 'Nu                 {:.10f}     Uniform kinematic viscosity gas\n'.format(nu))
    add_line(lines, 'Nudust             {:.10f}     Uniform kinematic viscosity dust\n'.format(nudust))
    if nubound is not None:
        add_line(lines, 'Nubound            {:.10f}     Uniform kinematic viscosity gas dependent on z with Nu = Nubound*8.5e-18*cosh(x*40/zmax)\n'.format(nubound))
    add_line(lines, 'OmegaFrame         1.0      Rotation rate\n')
    add_line(lines, 'ShearParam         1.5      Shear rate\n')

    if shear is not None:
            add_line(lines, 'Qz         {:.10f}      Vertical shear rate\n'.format(shear))
    ...
    if tracer is not None:
        add_line(lines, '\n### Tracer parameters\n\n')
        for i, tracer in enumerate(tracer):
            add_line(lines, 'Invstokestracer{}    {}     Inverse of the Stokes number for tracer1\n'.format(i, 1/tracer[0]))
            add_line(lines, 'Ftracer{}            {}     Fraction of tracer mass to gas mass\n'.format(i, tracer[1]))
            add_line(lines, 'Htracer{}            {}     Initial Scale Height tracer\n'.format(i, tracer[2]))
    ...
    add_line(lines, 'Ymin              ' + str(-box_size[1]) + '\n')
    add_line(lines, 'Ymax              ' + str(box_size[1]) + '\n')
    add_line(lines, 'Zmin              ' + str(-box_size[2]) + '\n')
    add_line(lines, 'Zmax              ' + str(box_size[2]) + '\n')
